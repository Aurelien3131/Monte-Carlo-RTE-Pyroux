...
class Detector:
    ...
    def merge(self, other): # Merges the results from another detector.
...
def detect(photon, det, histo,): # Function designed for a sensor that is always perpendicular to the z-axis      
    ...
    if (det.nz == -1) : # If the detector is facing z = 0
    	if (photon.z >= det.z): # The photon either passes by the detector or hits the detector
        	...
        	return True
    # REFLEXION
    if (det.nz == 1) : # If the detector is pointing toward z = zSize	
    	if (photon.z <= det.z): 
        	# 1.
...
def detect_polar(photon, det, histo, I_det, Q_det, U_det, V_det): # Function designed for a sensor that is always perpendicular to the z-axis      
    """
    Detector with a Stokes polarization analyzer.

    The detector first checks the geometrical and angular acceptance.
    If the photon is accepted, the detected intensity is calculated as:
    ...
    The detected intensity is then added to the polarization histogram.
    """
    ...
    if (det.nz == -1) : # If the detector is facing z = 0
    	if (photon.z >= det.z): # The photon either passes by the detector or hits the detector
        	...
        	histo.add_photon_polar(photon, arrival_time, I_det, Q_det, U_det, V_det)
...
class Histogramme:
    ...
    def add_photon_polar(self, photon, arrival_time = None, I_det = 1.0, Q_det = 0.0, U_det = 0.0, V_det = 0.0):
        if arrival_time == None :
            t = photon.timephot
        else : 
            t = arrival_time    
        ...
        # Stokes analyzer
        I_detected = 0.5 * ( I_det * photon.I +  Q_det * photon.Q + U_det * photon.U + V_det * photon.V )
        # Include the Monte Carlo photon weight
        detected_weight = photon.weight * I_detected
        
        # BEFORE TIME WINDOW
        if t < self.time_in:
            self.underflow_t = t
            self.underflow += detected_weight
            self.underflow_n += 1.0
            return
        ...
        if 0 <= idx < self.time_bin:
            self.hist[idx] += detected_weight
            self.numberphot[idx] += 1   

    def get(self): # Returns (centers of the bins, values)
        centers = 0.5 * (self.bins[:-1] + self.bins[1:])
        return centers, self.hist, self.numberphot
        
    def merge(self, other): # Adds a histogram from another process/workers.

        self.hist += other.hist
        self.numberphot += other.numberphot

        self.underflow += other.underflow
        self.overflow += other.overflow

        self.underflow_n += other.underflow_n
        self.overflow_n += other.overflow_n

        # Keep the outer edges (photons arriving before and after the time window) if useful
        self.underflow_t = min(self.underflow_t, other.underflow_t)
        self.overflow_t = max(self.overflow_t, other.overflow_t)  
