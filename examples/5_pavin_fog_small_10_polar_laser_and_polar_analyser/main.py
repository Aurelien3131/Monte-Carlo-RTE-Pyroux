...
for zSize in LZ_values: #  Loop over the simulation box thicknesses (Lz)
    ...
    for lambada in Lambda_values: # Loop over wavelength
        ...
        # Media parameters
            ...
            #idx_mat2 = box.add_material(mua= 0.0001, mus= mux, g=0, n=1, phase_file=None)     
            
                   
            # If one media
            box.set_region((-xSize, xSize), (-ySize, ySize), (0, zSize), idx_mat1)
        ...
        # ====================================================================================================================
            ...
            for pol_name, stokes in polar_inputs.items(): # boucle sur les polarisations d'entrÃ©es
                for freq in frequency :
                    for fact_modu in modulation :
                        print("LZ = ", zSize, "cm POL =", pol_name, "lambda = ", lambada, "µm visibilite = ", dense, "m frequency = ", freq, "Hz modulation = ", fact_modu)
                        time_window = 2*((nb_period/freq))
                        print("time in = ", t_ballistic-time_gate )
                        print ("time out = ", t_ballistic + time_window)
                        print ("time length signal =", nb_period/freq)
                ...
                # ==========================================================
                        ...
                        results = run_simulation( box, xSize, ySize, zSize, xdet, ydet, zdet, thetadet, phidet, diametre, NA, t_ballistic, time_gate, time_window, bins, freq, nb_period, boundary_function, source_function, source_params, stokes, stokes_det)
                ...
                # Create the “result” folder if it does not exist
                        ...
                        nom_fichier = ("result/log_small_10_freq_"+str(freq)+"_m_"+str(fact_modu)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                        nom_histo = ("result/histo_small_10_freq_"+str(freq)+"_m_"+str(fact_modu)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                        nom_histo_polar = ("result/histo_polar_small_10_freq_"+str(freq)+"_m_"+str(fact_modu)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                        nom_sub_histo = ("result/sub_histo_small_10_freq_"+str(freq)+"_m_"+str(fact_modu)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                        nom_emit_histo = ("result/histo_emit_small_10_freq_"+str(freq)+"_m_"+str(fact_modu)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                        ...
                        write_histogram( nom_histo_polar, results["Histogram_polar"], title="Histogramme temporel polar")
