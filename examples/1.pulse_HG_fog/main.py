...
for zSize in LZ_values: #  Loop over the simulation box thicknesses (Lz)
    ...
    for lambada in Lambda_values: # Loop over wavelength
        ...
        # Media parameters
            ...
            idx_mat2 = box.add_material(mua= 0.0001, mus= 100, g=0, n=1, phase_file=None)     
        ...
        # ====================================================================================================================
            ...
            for pol_name, stokes in polar_inputs.items(): # boucle sur les polarisations d'entr̩es
                ...
                nom_fichier = ("result/log_HG_freq_"+str(freq)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                nom_histo = ("result/histo_HG_freq_"+str(freq)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                nom_sub_histo = ("result/sub_histo_HG_freq_"+str(freq)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                nom_emit_histo = ("result/histo_emit_HG_freq_"+str(freq)+"_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
