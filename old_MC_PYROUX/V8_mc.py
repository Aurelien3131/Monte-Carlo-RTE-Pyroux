# MC_PYROUX_V8 : montecarlo core + one material + pencil beam + gaussian beam + detector + multimaterial + plane wave + condition cyclique x/y
    ...
    for lambada in Lambda_values:
        for dense in density_values:
            ...
            for pol_name, stokes in polar_inputs.items():
                ...
                histo = Histogramme(time_in = t_ballistic - 1e-9 , time_out = t_ballistic + time_window , time_bin=bins) #initialisation de l'histogramme temporel 

		# fichier enregistrement des photons emis
                nom_emit = ("liste_photon_emit_freq_"+str(int(freq/1e6))+"E6_emit_times_small_10_v_"+str(dense)+    "_l_"+str(lambada)+    "_z_"+str(zSize)+    "_pol_"+str(pol_name)+".txt")
                f_emit = open(nom_emit, "w", encoding="utf-8")
                f_emit.write("# photon_id\t t_emit(s)\n")
                ...
                for inphoton in range (1,Nphoton+1):
                	...
                	f_emit.write(f"{inphoton}\t{p.t_emit:.12e}\t{1.0 + math.cos(2 * math.pi * freq* p.t_emit):.12e}\n")
