# MC_PYROUX_V10 : montecarlo core + one material + pencil beam + gaussian beam + detector + multimaterial + plane wave + condition cyclique x/y
...
# + time + signal modulé + construit histogramme temporelle des photons émis + sous detecteur ballistique
...
c_velos = 299792458 # m/s
# ==========================================================
# Elements de boucles
# ==========================================================

polar_inputs = {"1000": (1,0,0,0)} #,"1001": (1,0,0,1),"1010": (1,0,1,0),"1100": (1,1,0,0)}
    
    
density_values = [40]#,35,47,65,124] #densité pavin big
Lambda_values = [9.8]#, 1.55, 4, 9.8]
LZ_values = [5000]#,500,1000,2000,3000,4000,5000] # pavin big et small #variation épaisseur brouillard
...
def propagate(photon):
	...
	time = distance/(c_velos*100)/old_n #distance en cm et c_velos en m/s donc *100
...
def launch_plane_wave(zinit):
    ...
    p.uy /= norm
    p.uz /= norm
    p.time = (p.n / (c_velos *100)) * (    (p.x - 0.0) * p.ux +    (p.y - 0.0) * p.uy +    (p.z - 0.0) * p.uz) # x y et z en cm et c_velos en m/s donc *100
    return p


def gaussian_source_modu(x0, y0, z0, w0, divergence,z_launch, ux, uy, uz, freq, nb_period, fact_modu):

	# temps d’émission (continu ou basé sur index photon)
	t_emit = random.random() * nb_period/ freq   # uniforme sur une période
	...
	return Photon(x=x, y=y, z=z, ux=ux, uy=uy, uz=uz, weight=weight, t_emit=t_emit, freq=freq)

# ==========================================================
# fonction detecteur

def detect(photon, det,histo):
    if (photon.z >= det.z): # le photon depasse le detecteur ou est sur le detecteur
        ...
        # 5. On met à jour le temps qu'a parcouru le photo après le détecteur et on le rajoute dans l'histogramme temporelle
        photon.timephot += t 
        histo.add_photon(photon)
...
class Photon:
	...
	# temps du photon
	timephot: float = 0.0
	freq: float = 1.0 # freqence pour la modulation
	t_emit: float = 0.0
...
class Histogramme:
    ...
    underflow_t: int = field(init=False)   # temps
    overflow_t: int = field(init=False)
    
    underflow: float = field(init=False)   # avant fenêtre (poids)
    overflow: float = field(init=False)    # après fenêtre (poids)

    underflow_n: int = field(init=False)   # nb photons
    overflow_n: int = field(init=False)
    
    def __post_init__(self):
        # Création des bornes des bins
        self.bins = np.linspace(self.time_in, self.time_out, self.time_bin + 1)
        # Histogramme initialisé à 0
        self.hist = np.zeros(self.time_bin)
        # Histogramme en nombre de photons
    ...
    def add_photon(self, photon):
        """Ajoute un photon dans l'histogramme selon son temps"""
        t = photon.timephot
    ...
    def get(self):
        """Retourne (centres des bins, valeurs)"""
...
# ==========================================================
Nphoton = 10000000	# nombre de photons à simuler
...
# parametre histogramme
bins = 2000
#parametre modulation	# modulation en I(to)=1+ fact_modu* cos(2*pi* freq * to) avec to = random( [0:nb_period] )
freq = 1e6 # [Hz]		
nb_period = 1  
fact_modu = 1 


time_window = 1 * nb_period / freq
time_gate =  time_window /10
...
for zSize in LZ_values:
    ...
    for lambada in Lambda_values:
        for dense in density_values:
            # initialisation des classes et prise en compte des parametres
            ...
            t_ballistic = (zdet-zinit)/ (100*c_velos/1.0) # facteur 100 car c en m/s et z en cm

            
            # time_in doit etre le temps minimum parcouru par un photon avec un mouvement purement ballistbiggggique à travers la boite
            # si 1 matériau alors tmin = ((zinit-zdet) / (c_velos*100) / 1) - 1e-6# avec 1 l'indice materiau
            # si 2 matériaux alors tmin = ((zinit-zmat_1) / (c_velos*100) / n1) +  ((zmat_1-zdet) / c_velos*100 / n2) - 1e-6 # avec n1 l'indice materiau 1 et n2 l'indice materiau 2 # z en cm et c_velos en m/s 
            for pol_name, stokes in polar_inputs.items():
                ...
                sub_detector = Detector(xdet,ydet,zdet,thetadet, phidet ,diametre,  0.05) #initialisation du sous-detecteur
                                
                histo_emit = HistogrammeEmission(time_in=0,    time_out=nb_period/freq,    time_bin=bins) #initialisation de l'histogramme temporel des photons emis 
                histo = Histogramme(time_in = t_ballistic - time_gate  , time_out = t_ballistic + time_window , time_bin=bins) #initialisation de l'histogramme temporel 
                sub_histo = Histogramme(time_in = t_ballistic - time_gate  , time_out = t_ballistic + time_window , time_bin=bins) #initialisation de l'histogramme temporel du sous-detecteur
                ...
                for inphoton in range (1,Nphoton+1):
                	...
                	p = gaussian_source_modu(xinit,yinit,zinit, W0, theta, zinit, uxinit, uyinit, uzinit, freq, nb_period, fact_modu) # source gaussienne modulée
                	histo_emit.add_emit(p)
                	...
                	while (p.alive ):
                		while (p.alive == True and p.stepLeft > 0):
                			...
                			if (p.sameVoxel == False):
                				detect(p,detector,histo) # si le photon s'est echappe par le cote du detecteur
                				detect(p,sub_detector,sub_histo) # si le photon s'est echappe par le cote du detecteur
                ...
                nom_fichier = ("log_small_10_freq_"+str(int(freq/1e6))+"E6_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                nom_histo = ("histo_small_10_freq_"+str(int(freq/1e6))+"E6_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                nom_sub_histo = ("sub_histo_small_10_freq_"+str(int(freq/1e6))+"E6_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")
                nom_emit_histo = ("histo_emit_small_10_freq_"+str(int(freq/1e6))+"E6_v_"+str(dense)+"_l_"+str(lambada)+"_z_" + str(zSize) + "_pol_" + str(pol_name) + ".txt")

		

                with open(nom_emit_histo, "w", encoding="utf-8") as f:

                    f.write("# Histogramme temporel d'emission\n")
                    f.write("# Temps(s)\tSignal\n")
                ...
                with open(nom_histo, "w", encoding="utf-8") as f_h:
                    f_h.write("# Histogramme temporel\n")

                    f_h.write("# Underflow (avant fenêtre)\n")
                    f_h.write(f"# poids = nb = {histo.underflow_t:.6e}, {histo.underflow:.6e}, nb = {histo.underflow_n}\n")

                    f_h.write("# Overflow (après fenêtre)\n")
                    f_h.write(f"# poids = {histo.overflow_t:.6e}, {histo.overflow:.6e}, nb = {histo.overflow_n}\n")
                    
                    f_h.write("# Temps(s)\tSignal\tNombre de Photons\n")
                    centers, values, numberphot = histo.get()

                    for t, v, l in zip(centers, values, numberphot):
                    	f_h.write(f"{t:.6e}\t{v:.6e}\t{l:.6e}\n")
                
                with open(nom_sub_histo, "w", encoding="utf-8") as f_hs:
                    f_hs.write("# Histogramme temporel du sous detecteur\n")

                    f_hs.write("# Underflow (avant fenêtre)\n")
                    f_hs.write(f"# poids = nb = {histo.underflow_t:.6e}, {histo.underflow:.6e}, nb = {histo.underflow_n}\n")

                    f_hs.write("# Overflow (après fenêtre)\n")
                    f_hs.write(f"# poids = {histo.overflow_t:.6e}, {histo.overflow:.6e}, nb = {histo.overflow_n}\n")
                    
                    f_hs.write("# Temps(s)\tSignal\tNombre de Photons\n")
                    centers, values, numberphot = sub_histo.get()
                    for t, v, l in zip(centers, values, numberphot):
                    	f_hs.write(f"{t:.6e}\t{v:.6e}\t{l:.6e}\n")
                ...
                with open(nom_fichier, "w", encoding="utf-8") as f:
                    ...
                    sub_C_rate = sub_detector.collected  / Nphoton            
                    f.write("\n" + "="*66 + "\n")
                    ...
                    # ----------------------------------------------------------
                    f.write("\n" + "-"*66 + "\n")
                    f.write("SOUS - DÉTECTEUR\n")
                    f.write("-"*66 + "\n")
                
                    f.write(f"{'Photons collectés':32s}: {sub_detector.collected:12.6f}\n")
                    f.write(f"{'Taux de collecte':32s}: {sub_C_rate:12.6f}\n")
                    f.write(f"{'Pourcentage collecté':32s}: {100*sub_C_rate:12.4f} %\n")              
