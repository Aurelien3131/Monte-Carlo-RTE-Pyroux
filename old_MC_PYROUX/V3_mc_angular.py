# MC_PYROUX_V3 : montecarlo core + one material + pencil beam + gaussian beam + detector + multimaterial + plane wave + filtre angulairement photon
...
xSize = 10		# [cm] boite va de -xSize à xSize
ySize = 10		# [cm] boite va de -ySize à ySize
zSize = 1.2		# [cm] boite va de 0 à zSize
Nx = 300		# nombre de voxel en x
Ny = 300		# nombre de voxel en y
Nz = 300 		# nombre de voxel en z
dx = 2 * xSize / Nx	# taille voxel en x
dy = 2 * ySize / Ny	# taille voxel en y 
dz = zSize / Nz		# taille voxel en z 
weightmin = 0.01 	# energie seuille pour roulette 
...
zinit = 0.1+0.0001		# position en z de la source en [cm]

# parametre de filtre angulaire
angular_deg = 60
mu_min = math.cos(math.radians(angular_deg)) 
mu_max = 1
...
c_velos = 2.99792458e10  # [cm/s]
diag_xy = math.sqrt((2*xSize)**2 + (2*ySize)**2)
time_old = (1000.0 / c_velos) * diag_xy
...
class Box:
    ...
    def add_material(self, mua, mus, g, n):
        """Ajoute un matériau et retourne son index"""
        self.materials.append({"mua": mua, "mus": mus, "g": g, "n": n})
...
def escape_periodic_xy(photon):
    ...
    Bords :
    - z < 0  => réflexion (R)
    - z > zSize => transmission (T)
    - x,y => périodique (cyclique)
    """
    ...
    return "inside"
def escape_periodic_xy_angular(photon,mu_min=0.0, mu_max=1.0):
    """
    Bords :
    - z < 0  => réflexion (R)
    - z > zSize => transmission (T)
...
def propagate(photon):
	...
	photon.time += s * photon.n / c_velos
	...
	if photon.sameVoxel == False:
		...
		# Calcul du nouveau voxel manuellement depuis old + direction
		# NE PAS utiliser photon.ix/iy/iz ici : update_voxel() n'a pas encore tourné
		new_ix = old_ix
		new_iy = old_iy
		new_iz = old_iz

		if abs(s - photon.tx) < eps:
			new_ix = old_ix + (1 if photon.ux > 0 else -1)
		if abs(s - photon.ty) < eps:
			new_iy = old_iy + (1 if photon.uy > 0 else -1)
		if abs(s - photon.tz) < eps:
			new_iz = old_iz + (1 if photon.uz > 0 else -1)

		# Si hors grille : pas de Fresnel, escape() s'en occupera
		if (0 <= new_ix < Nx and 0 <= new_iy < Ny and 0 <= new_iz < Nz):

			new_idx = box.voxel_map[new_ix][new_iy][new_iz]
			...
			if mu != 1:
				...
				# Normale à l'interface : construite depuis la face traversée
				...
				cos_in = abs(nx * photon.ux + ny * photon.uy + nz * photon.uz)
				
				
				

    				# CAS mu > 1 : n1 > n2, réflexion totale interne possible
    				# angle limite : theta_c = arcsin(1/mu)
   			 	# si cos_in < sqrt(1 - 1/mu²) alors theta_in > theta_c
				if mu > 1:
					theta_c  = math.asin(1.0 / mu)
					cos_limit = math.cos(theta_c)           # = sqrt(1 - 1/mu²)
					#print(f"  mu>1 : angle limite = {math.degrees(theta_c):.2f}°, cos_in = {cos_in:.4f}, cos_limit = {cos_limit:.4f}")

					if cos_in < cos_limit:
						# theta_in > theta_c : réflexion totale interne
						#print("  --> REFLEXION TOTALE INTERNE")
						photonReflected = True
					else:
						# theta_in <= theta_c : Fresnel normal
						cos_out_sqr = 1.0 - mu**2 * (1.0 - cos_in**2)
						cos_out     = math.sqrt(max(0.0, cos_out_sqr))
						Rs = ((cos_in - mu * cos_out) / (cos_in + mu * cos_out))**2
						Rp = ((mu * cos_in - cos_out) / (mu * cos_in + cos_out))**2
						R  = (Rs + Rp) / 2.0
						#print(f"  --> Fresnel mu>1 : R = {R:.4f}")
						photonReflected = random.random() <= R
				

				# CAS mu < 1 : n1 < n2, pas de réflexion totale interne
				# cos_out_sqr toujours > 0, Fresnel direct
				elif mu < 1:
					cos_out_sqr = 1.0 - mu**2 * (1.0 - cos_in**2)
					cos_out     = math.sqrt(max(0.0, cos_out_sqr))
					Rs = ((cos_in - mu * cos_out) / (cos_in + mu * cos_out))**2
				...
				# Application réflexion ou réfraction
				if photonReflected:
					# Réflexion : inverser la composante selon la normale
					dot = nx * photon.ux + ny * photon.uy + nz * photon.uz
					photon.ux -= 2 * dot * nx
					photon.uy -= 2 * dot * ny
					photon.uz -= 2 * dot * nz
					# photon.n reste inchangé

				else:
					# Réfraction : loi de Snell vectorielle
					cos_out_sqr = 1.0 - mu**2 * (1.0 - cos_in**2)
					cos_out     = math.sqrt(max(0.0, cos_out_sqr))
					dot    = nx * photon.ux + ny * photon.uy + nz * photon.uz
					sign   = 1.0 if dot >= 0 else -1.0
					ncoeff = mu * dot - sign * cos_out
					photon.ux = mu * photon.ux - ncoeff * nx
					photon.uy = mu * photon.uy - ncoeff * ny
					photon.uz = mu * photon.uz - ncoeff * nz
					photon.n  = new_n		
	# 3. Absorption
...
def launch_plane_wave(zinit):
    ...
    p.uy /= norm
    p.uz /= norm
    p.time = (p.n / c_velos) * (    (p.x - 0.0) * p.ux +    (p.y - 0.0) * p.uy +    (p.z - 0.0) * p.uz)
...
idx_mat1 = box.add_material(mua=1e-16, mus=1e-16, g=1, n=1)

# matériau 2 : materiau 2
idx_mat2 = box.add_material(mua=0.1, mus=45, g=0.8, n=1.4)

box.set_region((-xSize, xSize), (-ySize, ySize), (0, 0.1), idx_mat1)
box.set_region((-xSize, xSize), (-ySize, ySize), (0.1, 1.1), idx_mat2)
box.set_region((-xSize, xSize), (-ySize, ySize), (1.1, zSize), idx_mat1)
	...
	while (p.alive ):
		while (p.alive == True and p.stepLeft > 0):
			...
			if (p.sameVoxel == False):
				...
				escape_periodic_xy_angular(p,mu_min,mu_max) # comme escape mais periodique en x et y et filtre en angle
...
# ============================
# Ecriture fichier (append)
# ============================

# paramètres pour nom de fichier
n_mid = 1
z_label = zinit - 0.1

# format propre du float
z_str = f"{z_label:.5f}"

filename = f"RTA_inside_n_mid_{n_mid}_LO_{z_str}_.txt"

# calcul des grandeurs
R = compteurR / Nphoton
T = compteurT / Nphoton
A = 1 - (compteurR + compteurT) / Nphoton


# écriture en append
with open(filename, "a") as f:
    f.write(f"1\t{R}\t{T}\t{A}\t{angular_deg}\n")
